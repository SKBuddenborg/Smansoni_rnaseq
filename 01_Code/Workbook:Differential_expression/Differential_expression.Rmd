...
#sampleSex <- substr(sampleCondition, 1, 1)
#sampleTableSex <- data.frame(sampleName=sampleName,
...
```{r}
directory <- ('/Users/skb/Desktop/Smansoni_rnaseq')
sampleFiles <- grep ("_fixhtseq.o", list.files(directory),value=TRUE)
sampleCondition <- gsub("_R._fixhtseq.o","",sampleFiles)
...
ddsHTSeq$condition <- factor(ddsHTSeq$condition, levels = (unique(sampleCondition)))
```

# Generate DESeq results table for all groups with FDR cutoff of 0.05
```{r}
dds$eggmira <- factor (ddsHTSeq$condition, levels=c("X_Eggs","X_Miracidia"))
dds$mira1spor <- factor (ddsHTSeq$condition, levels=c("X_Miracidia","X_1d_Sporocysts"))
dds$spor5spor <- factor (ddsHTSeq$condition, levels=c("X_1d_Sporocysts","X_5d_Sporocysts"))
dds$spor32spor <- factor (ddsHTSeq$condition, levels=c("X_5d_Sporocysts","X_32d_Sporocysts"))
dds$sporcerc <- factor (ddsHTSeq$condition, levels=c("X_32d_Sporocysts","X_Cercariae"))
dds$cercsom <- factor (ddsHTSeq$condition, levels=c("X_Cercariae","X_2d_Somules"))
dds$somjuv <- factor (ddsHTSeq$condition, levels=c("X_2d_Somules","X_26d_juveniles"))
